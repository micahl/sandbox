...
# note, the command to call this is something like: 		/bin/bash -c "wget -O - https://raw.githubusercontent.com/micahl/sandbox/main/azurebatch/buildyocto.sh | bash"

# ----------------------
# user defined settings
# ----------------------

IOTEDGE_BRANCH=ggjjj/support-for-1.4
BUILD=dunfell
...
sudo apt-get install -y ca-certificates curl apt-transport-https lsb-release gnupg gawk wget git diffstat unzip texinfo gcc build-essential chrpath socat cpio python3 python3-pip python3-pexpect xz-utils debianutils iputils-ping python3-git python3-jinja2 libegl1-mesa libsdl1.2-dev pylint3 xterm python3-subunit mesa-common-dev zstd liblz4-tool
...
git clone -b $BUILD http://git.yoctoproject.org/git/meta-raspberrypi
#git clone -b $BUILD https://github.com/Azure/meta-iotedge.git
git clone -b $IOTEDGE_BRANCH https://github.com/Azure/meta-iotedge.git
...
# note RUST may have been updated to a newer version, which could be incompatible
...
echo "BBLAYERS += \"$TARGETDIR/source/meta-raspberrypi\"" >> yocto/conf/bblayers.conf
...
echo 'IMAGE_INSTALL_append += " aziot-edged iotedge aziotd aziot-keys aziotctl "' >> yocto/conf/local.conf
